...
library Pool {
    ...
    function initialize(State storage self, uint160 sqrtPriceX96, uint24 lpFee)
        internal
        returns (int24 tick)
    {
        ...
        self.slot0 =
            Slot0.wrap(bytes32(0)).setSqrtPriceX96(sqrtPriceX96).setTick(tick).setLpFee(lpFee);
    }
...

        {
...

            {
                ...
                Position.State storage position =
                    self.positions.get(params.owner, tickLower, tickUpper, params.salt);
        ...
        )
    {
        ...
        step.feeGrowthGlobalX128 =
            zeroForOne ? self.feeGrowthGlobal0X128 : self.feeGrowthGlobal1X128;
        ...
        while (!(amountSpecifiedRemaining == 0
            ...
            (step.tickNext, step.initialized) = self.tickBitmap
                .nextInitializedTickWithinOneWord(result.tick, params.tickSpacing, zeroForOne);
            ...
            (result.sqrtPriceX96, step.amountIn, step.amountOut, step.feeAmount) =
                SwapMath.computeSwapStep(
                    ...
                    SwapMath.getSqrtPriceTarget(
                        zeroForOne, step.sqrtPriceNextX96, params.sqrtPriceLimitX96
                    ),
                    result.liquidity,
            ...
            if (result.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    int128 liquidityNet = Pool.crossTick(
                        self, step.tickNext, feeGrowthGlobal0X128, feeGrowthGlobal1X128
                    );
        ...
        returns (BalanceDelta delta)
    {
        ...
        unchecked {
            ...
            if (amount0 > 0) {
                state.feeGrowthGlobal0X128 += UnsafeMath.simpleMulDiv(
                    amount0, FixedPoint128.Q128, liquidity
                );
            }
            if (amount1 > 0) {
                state.feeGrowthGlobal1X128 += UnsafeMath.simpleMulDiv(
                    amount1, FixedPoint128.Q128, liquidity
                );
            }
        }
    }
        ...
        returns (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128)
    {
        ...
        unchecked {
            if (tickCurrent < tickLower) {
                ...
                feeGrowthInside1X128 = self.feeGrowthGlobal1X128 - lower.feeGrowthOutside1X128
