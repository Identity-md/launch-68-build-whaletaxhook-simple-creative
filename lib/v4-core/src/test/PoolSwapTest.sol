...
contract PoolSwapTest is PoolTestBase {
    ...
    function swap(
        ...
        delta = abi.decode(
            manager.unlock(
                abi.encode(CallbackData(msg.sender, testSettings, key, params, hookData))
            ),
            (BalanceDelta)
        );
    ...
    function unlockCallback(bytes calldata rawData) external returns (bytes memory) {
        ...
        if (data.params.zeroForOne) {
            if (data.params.amountSpecified < 0) {
                ...
                require(
                    delta.amount0() == deltaAfter0, "delta.amount0() is not equal to deltaAfter0"
                );
                ...
                require(
                    delta.amount1() == deltaAfter1, "delta.amount1() is not equal to deltaAfter1"
                );
                require(
        ...
        if (deltaAfter0 > 0) {
            data.key.currency0
                .take(manager, data.sender, uint256(deltaAfter0), data.testSettings.takeClaims);
        }
        if (deltaAfter1 > 0) {
            data.key.currency1
                .take(manager, data.sender, uint256(deltaAfter1), data.testSettings.takeClaims);
        }
