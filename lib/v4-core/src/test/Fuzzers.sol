...
contract Fuzzers is StdUtils {
    ...
    function boundLiquidityDelta(
        PoolKey memory key,
        int256 liquidityDeltaUnbounded,
        int256 liquidityMaxByAmount
    ) internal pure returns (int256) {
        int256 liquidityMaxPerTick = int256(
            uint256(Pool.tickSpacingToMaxLiquidityPerTick(key.tickSpacing))
        );

        // Finally bound the seeded liquidity by either the max per tick, or by the amount allowed in the position range.
        int256 liquidityMax =
            liquidityMaxByAmount > liquidityMaxPerTick ? liquidityMaxPerTick : liquidityMaxByAmount;
    ...
    function boundLiquidityDeltaTightly(
        ...
        // Finally bound the seeded liquidity by either the max per tick, or by the amount allowed in the position range.
        int256 liquidityMax = liquidityMaxByAmount > liquidityMaxTightBound
            ? liquidityMaxTightBound
    ...
    function boundTicks(int24 tickLower, int24 tickUpper, int24 tickSpacing)
        internal
        pure
        returns (int24, int24)
    {
        ...
        (tickLower, tickUpper) =
            tickLower < tickUpper ? (tickLower, tickUpper) : (tickUpper, tickLower);

        if (tickLower == tickUpper) {
            if (tickLower != TickMath.minUsableTick(tickSpacing)) {
    ...
    function boundTicks(PoolKey memory key, int24 tickLower, int24 tickUpper)
        internal
        pure
        returns (int24, int24)
    {
    ...
    function createFuzzyLiquidityParams(
        ...
        result.liquidityDelta =
            boundLiquidityDelta(key, params.liquidityDelta, liquidityDeltaFromAmounts);
    }
    ...
    function createFuzzyLiquidityParamsWithTightBound(
        ...
        result.liquidityDelta = boundLiquidityDeltaTightly(
