...
contract DeltaReturningHook is BaseTestHooks {
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        if (deltaUnspecifiedBeforeSwap != 0) {
            _settleOrTake(unspecifiedCurrency, deltaUnspecifiedBeforeSwap);
        }

        BeforeSwapDelta beforeSwapDelta =
            toBeforeSwapDelta(deltaSpecified, deltaUnspecifiedBeforeSwap);
