...
contract PoolDonateTest is PoolTestBase {
    ...
    {
        delta = abi.decode(
            manager.unlock(abi.encode(CallbackData(msg.sender, key, amount0, amount1, hookData))),
            (BalanceDelta)
        );
    ...
    function unlockCallback(bytes calldata rawData) external returns (bytes memory) {
        ...
        require(
            deltaAfter0 == -int256(data.amount0),
            "deltaAfter0 is not equal to -int256(data.amount0)"
        );
        require(
            deltaAfter1 == -int256(data.amount1),
            "deltaAfter1 is not equal to -int256(data.amount1)"
        );
