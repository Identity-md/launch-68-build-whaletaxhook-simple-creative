...
contract PoolManagerInitializeTest is Test, Deployers {
    ...
    function test_fuzz_initialize(PoolKey memory key0, uint160 sqrtPriceX96) public {
        ...
        if (key0.tickSpacing > TickMath.MAX_TICK_SPACING) {
            vm.expectRevert(
                abi.encodeWithSelector(IPoolManager.TickSpacingTooLarge.selector, key0.tickSpacing)
            );
            manager.initialize(key0, sqrtPriceX96);
        } else if (key0.tickSpacing < TickMath.MIN_TICK_SPACING) {
            vm.expectRevert(
                abi.encodeWithSelector(IPoolManager.TickSpacingTooSmall.selector, key0.tickSpacing)
            );
            manager.initialize(key0, sqrtPriceX96);
            ...
            vm.expectRevert(
                abi.encodeWithSelector(Hooks.HookAddressNotValid.selector, address(key0.hooks))
            );
            manager.initialize(key0, sqrtPriceX96);
    ...
    function test_initialize_succeedsWithHooks(uint160 sqrtPriceX96) public {
        ...
        address payable mockAddr =
            payable(address(uint160(Hooks.BEFORE_INITIALIZE_FLAG | Hooks.AFTER_INITIALIZE_FLAG)));
        ...
        assertEq(
            MockContract(mockAddr).timesCalledSelector(beforeSelector), 1, "beforeSelector count"
        );
        assertTrue(
            MockContract(mockAddr).calledWithSelector(beforeSelector, beforeParams),
            "beforeSelector params"
        );
        assertEq(
            MockContract(mockAddr).timesCalledSelector(afterSelector), 1, "afterSelector count"
        );
        assertTrue(
            MockContract(mockAddr).calledWithSelector(afterSelector, afterParams),
            "afterSelector params"
        );
    }
    ...
    function test_initialize_failsWithIncorrectSelectors() public {
        address hookAddr =
            address(uint160(Hooks.BEFORE_INITIALIZE_FLAG | Hooks.AFTER_INITIALIZE_FLAG));
        ...
        mockHooks.setReturnValue(
            mockHooks.beforeInitialize.selector, mockHooks.beforeInitialize.selector
        );
        vm.expectRevert(Hooks.InvalidHookResponse.selector);
        manager.initialize(uninitializedKey, SQRT_PRICE_1_1);
    }
    ...
    function test_initialize_succeedsWithCorrectSelectors() public {
        ...
        mockHooks.setReturnValue(
            mockHooks.afterInitialize.selector, mockHooks.afterInitialize.selector
        );
    ...
    function test_initialize_failsIfTickSpaceTooLarge(uint160 sqrtPriceX96) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IPoolManager.TickSpacingTooLarge.selector, uninitializedKey.tickSpacing
            )
        );
        manager.initialize(uninitializedKey, sqrtPriceX96);
    }
    ...
    function test_initialize_failsIfTickSpaceZero(uint160 sqrtPriceX96) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IPoolManager.TickSpacingTooSmall.selector, uninitializedKey.tickSpacing
            )
        );
        manager.initialize(uninitializedKey, sqrtPriceX96);
    }
