...
contract TickMathTestTest is Test, JavascriptFfi {
    ...
    function test_getSqrtPriceAtTick_isValidMaxTick() public view {
        ...
        assertEq(
            tickMath.getSqrtPriceAtTick(MAX_TICK), 1461446703485210103287273052203988822378723970342
        );
    }
    ...
    function test_getTickAtSqrtPrice_isValidMaxSqrtPriceMinusOne() public view {
        assertEq(
            tickMath.getTickAtSqrtPrice(1461373636630004318706518188784493106690254656249),
            MAX_TICK - 1
        );
    }
    ...
    function test_getTickAtSqrtPrice_matchesJavascriptImplWithin1() public {
        ...
        unchecked {
            while (sqrtPrice < sqrtPrice * 16) {
                if (sqrtPrice != MIN_SQRT_PRICE) {
                    jsParameters = string(abi.encodePacked(jsParameters, ",")); // do not leave comma in front of first number
                }
                // add tick to javascript parameters to be calculated inside script
        ...
        for (uint256 i = 0; i < jsTicks.length; i++) {
            ...
            (int24 gtResult, int24 ltResult) =
                jsTick > solTick ? (jsTick, solTick) : (solTick, jsTick);
    ...
    function test_fuzz_getTickAtSqrtPrice_getSqrtPriceAtTick_relation(int24 tick) public pure {
        ...
        assertEq(
            TickMath.getTickAtSqrtPrice(
                uint160((uint256(priceAtTick) + uint256(priceAtNextTick)) / 2)
            ),
            tick,
            "mid price"
        );
