...
contract SkipCallsTest is Test, Deployers {
    ...
    function deploy(SkipCallsTestHook skipCallsTestHook) private {
        ...
        (key,) = initPool(
            currency0, currency1, IHooks(address(skipCallsTestHook)), 3000, SQRT_PRICE_1_1
        );
    }

    function approveAndAddLiquidity(SkipCallsTestHook skipCallsTestHook) private {
        MockERC20(Currency.unwrap(key.currency0))
            .approve(address(skipCallsTestHook), Constants.MAX_UINT256);
    ...
    function test_beforeRemoveLiquidity_skipIfCalledByHook() public {
        ...
        modifyLiquidityRouter.modifyLiquidity(
            key, REMOVE_LIQUIDITY_PARAMS, abi.encode(address(this))
        );
        assertEq(skipCallsTestHook.counter(), 1);
    ...
    function test_afterSwap_skipIfCalledByHook() public {
        SkipCallsTestHook skipCallsTestHook = SkipCallsTestHook(
            address(
                uint160(type(uint160).max & clearAllHookPermissionsMask | Hooks.AFTER_SWAP_FLAG)
            )
        );
    ...
    function test_beforeDonate_skipIfCalledByHook() public {
        SkipCallsTestHook skipCallsTestHook = SkipCallsTestHook(
            address(
                uint160(type(uint160).max & clearAllHookPermissionsMask | Hooks.BEFORE_DONATE_FLAG)
            )
        );
    ...
    function test_afterDonate_skipIfCalledByHook() public {
        SkipCallsTestHook skipCallsTestHook = SkipCallsTestHook(
            address(
                uint160(type(uint160).max & clearAllHookPermissionsMask | Hooks.AFTER_DONATE_FLAG)
            )
        );
