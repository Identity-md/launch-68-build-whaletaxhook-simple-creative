...
contract TestCurrency is Test {
    ...
    function test_fuzz_fromId_toId_opposites(Currency currency) public view {
        assertEq(
            ...
            Currency.unwrap(currencyTest.fromId(currencyTest.toId(currency)))
        );
    }

    function test_fuzz_toId_fromId_opposites(uint256 id) public view {
        assertEq(
            id & 0x00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF,
            currencyTest.toId(currencyTest.fromId(id))
        );
    }
