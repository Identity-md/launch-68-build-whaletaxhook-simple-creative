...
contract CustomAccountingTest is Test, Deployers {
    ...
    function _etchHookAndInitPool(address hookAddr, address implAddr) internal {
        ...
        (key,) =
            initPoolAndAddLiquidity(currency0, currency1, IHooks(hookAddr), 100, SQRT_PRICE_1_1);
    }
    ...
    function test_fuzz_swap_beforeSwap_returnsDeltaSpecified(
        ...
        hookDeltaSpecified =
            int128(bound(hookDeltaSpecified, type(int128).min + 1, int128(reservesOfSpecified)));
        ...
        if (amountSpecified == 0) {
            ...
            if (isExactIn && (hookDeltaSpecified + amountSpecified < maxPossibleIn_fuzz_test)) {
                ...
                assertEq(
                    deltaSpecified,
                    maxPossibleIn_fuzz_test - hookDeltaSpecified,
                ...
                !isExactIn && (hookDeltaSpecified + amountSpecified > maxPossibleOut_fuzz_test)
            ) {
                // the hook will have taken hookDeltaSpecified of the maxPossibleOut
                assertEq(
                    deltaSpecified,
                    maxPossibleOut_fuzz_test - hookDeltaSpecified,
    ...
    function test_fuzz_addLiquidity_withLPFeeTakingHook(uint128 feeRevenue0, uint128 feeRevenue1)
        public
    {
    ...
    function test_fuzz_removeLiquidity_withLPFeeTakingHook(uint128 feeRevenue0, uint128 feeRevenue1)
        public
    {
