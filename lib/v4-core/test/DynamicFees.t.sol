...
contract TestDynamicFees is Test, Deployers {
    ...
    DynamicFeesTestHook dynamicFeesNoHooks = DynamicFeesTestHook(
        address(uint160(uint256(type(uint160).max) & clearAllHookPermissionsMask))
    );
    ...
    function test_updateDynamicLPFee_beforeSwap_succeeds_gas() public {
        ...
        emit Swap(
            key.toId(), address(swapRouter), -100, 98, 79228162514264329749955861424, 1e18, -1, 123
        );
    ...
    function test_swap_100PercentFee_AmountIn_WithProtocol() public {
        ...
        SwapParams memory params = SwapParams({
            zeroForOne: true, amountSpecified: -1000, sqrtPriceLimitX96: SQRT_PRICE_1_2
        });
        PoolSwapTest.TestSettings memory testSettings =
            PoolSwapTest.TestSettings({takeClaims: false, settleUsingBurn: false});
    ...
    function test_emitsSwapFee() public {
        ...
        vm.expectEmit(true, true, true, true, address(manager));
        emit Swap(
            key.toId(), address(swapRouter), -100, 98, 79228162514264329749955861424, 1e18, -1, 1123
        );
    ...
    function test_fuzz_ProtocolAndLPFee(
        ...
        if (lpFee == 0) {
            ...
            if (((uint256(uint128(-delta.amount0())) * protocolFee0) % 1e6) != 0) {
                expectedProtocolFee++;
            }
        }
