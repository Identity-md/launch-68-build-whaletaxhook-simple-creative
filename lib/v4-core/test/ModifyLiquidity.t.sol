...
contract ModifyLiquidityTest is Test, Logger, Deployers, JavascriptFfi, Fuzzers {
    ...
    function setUp() public {
        ...
        (simpleKey, simplePoolId) =
            initPool(currency0, currency1, IHooks(address(0)), 3000, SQRT_PRICE_1_1);
    }
    ...
    function _modifyLiquidityJS(PoolId poolId, ModifyLiquidityParams memory params)
        public
    ...
    function test_modifyLiquidity_samePosition_zeroSalt_isUpdated() public {
        (uint128 liquidity,,) = manager.getPositionInfo(
            ...
            LIQ_PARAM_NO_SALT.tickLower,
            LIQ_PARAM_NO_SALT.tickUpper,
            0
        );
    ...
    function test_modifyLiquidity_samePosition_withSalt_isUpdated() public {
        (uint128 liquidity,,) = manager.getPositionInfo(
            simplePoolId,
            address(modifyLiquidityRouter),
            LIQ_PARAM_SALT.tickLower,
            LIQ_PARAM_SALT.tickUpper,
            SALT
        );
        assertEq(liquidity, 0);
    ...
    function test_modifyLiquidity_sameTicks_withDifferentSalt_isNotUpdated() public {
        ...
        assertEq(
            updatedLiquidityWithSalt,
            liquiditySalt + uint128(uint256(LIQ_PARAM_SALT.liquidityDelta))
        );
        assertGt(updatedLiquidityWithSalt, updatedLiquidityNoSalt);
        assertEq(updatedLiquidityNoSalt, liquidityNoSalt);
    }

    function test_modifyLiquidity_sameSalt_differentLiquidityRouters_doNotEditSamePosition()
        public
    {
        // Set up new router.
        ...
        MockERC20(Currency.unwrap(currency0))
            .approve(address(modifyLiquidityRouter2), Constants.MAX_UINT256);
        MockERC20(Currency.unwrap(currency1))
            .approve(address(modifyLiquidityRouter2), Constants.MAX_UINT256);

        ModifyLiquidityParams memory LIQ_PARAM_SALT_2 = ModifyLiquidityParams({
            tickLower: -120, tickUpper: 120, liquidityDelta: 2e18, salt: SALT
        });
        ...
        (uint128 liquiditySalt2,,) = manager.getPositionInfo(
            simplePoolId,
            address(modifyLiquidityRouter2),
            LIQ_PARAM_SALT_2.tickLower,
            LIQ_PARAM_SALT_2.tickUpper,
            SALT
        );
